...
class MultifieldDataSampler( torch.utils.data.IterableDataset):
  ...
  def __init__( self, file_path, years_data, fields, batch_size, 
    ...
    print(f"res: {self.res}, pole_offset: {pole_offset}, is_global:{self.is_global}")
    print(f"range_lat: {self.range_lat}, range_lon: {self.range_lon}")
    ...
    print(f"not global range_lat: {self.range_lat}, not global range_lon: {self.range_lon}")
    print(f"__________________________________________________________________")
  ...
  def shuffle( self) :
    ...
    for i_ym in range( len(self.years_months)) :
      ...
      #print(f"USING SHUFFLE IDXS_PERM")
      #print(f"idxs_perm{self.idxs_perm}")
