...
class DynamicFieldLevel() :
  ...
  def __init__( self, file_path, years_data, field_info,
    ...
    print(f"FILE_GEO_RANGE_START: {file_geo_range}")
    self.file_geo_range = [ -np.array(file_geo_range[0]) + 90. , np.array(file_geo_range[1]) ]
    print(f"FILE_GEO_RANGE_CONVERTED: {self.file_geo_range}")
    ...
    print(f"FILE_GEO_RANGE_END: {self.file_geo_range}")
  ...
  def load_data( self, years_months, idxs_perm, batch_size = None) :
    ...
    for j in range( len(self.data_field) ) :
      #print(f"self.corr_type: {self.corr_type}")
      #print(f"self.field_info: {self.field_info}")
      #print(f"self.file_geo_range: {self.file_geo_range}")
  ...
  def __getitem__( self, bidx) :
    ...
    for jj in range( self.batch_size) :
      ...
      #print(f"geor: {geor}, idx: {idx}, jj: {jj}, CID: {cid}")
      #print(f"geor[0][0]: {geor[0][0] } <= cid[1]: {cid[1]} <= geor[0][1]: {geor[0][1]} ==> {geor[0][0] <= cid[1] <= geor[0][1]}")
      ...
      if self.is_global :
        ...
        #print(f"ran_lon = {ran_lon}, file_shape = {self.file_shape} ==>{all(ran_lon >= 0) and all(ran_lon < self.file_shape[2])}")
      ...
      print(f"i_ym: {i_ym}, ran_t:{ran_t}, ran_lat: {ran_lat}, ran_lon: {ran_lon}")
